...
class UserRepository extends BaseRepository implements UserRepositoryInterface
{
    ...
    {
        if ($isFullList === true) {
            $results = $this->model->select('id', 'name', 'email', 'phone', 'role_id', 'type', 'work_mail', 'work_mail_pass')->with('role')->get();
            return [
                'data' => $results,
                'total' => $results->count()
            ];
        ...
        $queryBuilder = $queryBuilder->select('id', 'name', 'email', 'phone', 'role_id', 'type', 'work_mail', 'work_mail_pass')->with(['role' => function($query) {
